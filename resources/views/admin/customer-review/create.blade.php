...
<div class="grid grid-cols-1 xl:grid-cols-12 gap-x-5 mt-4">
    <div class="xl:col-span-12">
        <div class="card">
            <div class="card-body">
                <h6 class="mb-4 text-15">{{ __('Thêm đánh giá khách hàng') }}</h6>

                <form method="POST" action="{{ route('admin.customer.review.store') }}" enctype="multipart/form-data">
                    {{ csrf_field() }}
                    <div class="grid grid-cols-1 gap-5 lg:grid-cols-2 xl:grid-cols-12">
                        <div class="xl:col-span-6">
                            <label for="customer_name" class="inline-block mb-2 text-base font-medium">{{ __('Tên khách hàng') }}</label>
                        ...
                        <div class="xl:col-span-6">
                            <label for="customer_review" class="inline-block mb-2 text-base font-medium">{{ __('Nội dung đánh giá') }}</label>
                            <textarea type="text" id="customer_review" name="customer_review" value="{{ old('customer_review') }}" class="form-input  border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200" placeholder="{{ __('Nội dung đánh giá') }}"></textarea>
                    ...
                    <div class="flex justify-end gap-2 mt-4">
                        ...
                        <button type="submit" class="text-white btn bg-custom-500 border-custom-500 hover:text-white hover:bg-custom-600 hover:border-custom-600 focus:text-white focus:bg-custom-600 focus:border-custom-600 focus:ring focus:ring-custom-100 active:text-white active:bg-custom-600 active:border-custom-600 active:ring active:ring-custom-100 dark:ring-custom-400/20">{{ __('Thêm đánh giá') }}</button>
                    </div>
                </form>
            </div>
        </div>
