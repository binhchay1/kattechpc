                ...
                <form method="POST" action="{{route('admin.promotion.update', $promotion['id'])}}" enctype="multipart/form-data">
                    ...
                    <div class="grid grid-cols-1 gap-5 lg:grid-cols-2 xl:grid-cols-12">
                        ...
                        <div class="xl:col-span-6">
                            ...
                            <textarea class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200" id="productDescription" name="content" placeholder="{{__('Nội dung bài viết')}}" rows="5">{!! html_entity_decode($promotion->content)!!}</textarea>
                            @if ($errors->has('content'))
                            <span class="text-danger">{{ $errors->first('content') }}</span>
                            @endif
                        </div>

                        <input type="hidden" name="slug" class="form-control" id="name" placeholder="Enter name" value="{{ $promotion->slug }}">
                    </div>
